...
// Blog yazısını getir
$stmt = $db->prepare("SELECT * FROM blog_posts WHERE slug = ? AND is_active = 1");
$stmt->execute([$slug]);
$blog = $stmt->fetch();
...
$meta_description = !empty($blog['meta_description']) ? $blog['meta_description'] : $blog['excerpt'];
...
<head>
    ...
    <meta property="og:image" content="<?php echo SITE_URL . '/' . $blog['image']; ?>">
...
<?php endif; ?>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .blog-image { width: 100%; max-width: 100%; height: auto; border-radius: 8px; margin: 30px 0; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        .blog-content { max-width: 900px; margin: 0 auto; line-height: 1.8; font-size: 1.1rem; }
        .blog-content h1, .blog-content h2, .blog-content h3 { color: var(--primary-color); margin-top: 30px; margin-bottom: 15px; }
        .blog-content p { margin-bottom: 20px; }
        .blog-content ul, .blog-content ol { margin-left: 20px; margin-bottom: 20px; }
        .blog-meta { text-align: center; color: #666; margin-bottom: 30px; font-size: 14px; }
...
<body>
    <div class="sticky-icons-bottom-left">
        ...
        <a href="https://wa.me/<?php echo $whatsapp; ?>?text=Merhaba,%20<?php echo urlencode($blog['title']); ?>%20hakkında%20bilgi%20almak%20istiyorum." target="_blank" class="whatsapp-icon" aria-label="WhatsApp ile İletişime Geç"><i class="fab fa-whatsapp"></i></a>
        <?php endif; ?>
        <?php if (!empty($phone)): ?>
    ...
    <div class="page-header">
        <div class="container">
            ...
            <div class="breadcrumb">
                ...
                <span>Blog</span>
                <span>/</span>
                <span><?php echo clean($blog['title']); ?></span>
            </div>
        </div>
    </div>
    ...
    <section class="content-section">
        <div class="container">
            <div class="blog-content">
                <div class="blog-meta">
                    <i class="fas fa-calendar-alt"></i> <?php echo formatDate($blog['created_at']); ?>
                </div>

                <img src="<?php echo $blog['image']; ?>" alt="<?php echo clean($blog['title']); ?>" class="blog-image">

                <?php echo $blog['content']; ?>

                <div class="cta-box">
                    <h2><i class="fas fa-phone"></i> <?php echo clean($blog['title']); ?> İçin Bizi Arayın!</h2>
                    <p>Bölgenize özel profesyonel nakliye hizmetlerimiz hakkında detaylı bilgi ve fiyat teklifi için hemen bizi arayın.</p>
                    <?php if (!empty($phone)): ?>
